...
public class JDBCRunner4 implements CommandLineRunner {
    private static final Logger LOGGER = LoggerFactory.getLogger(JDBCRunner4.class.getName());
    @Autowired
    ...
    public void queryDataByUsername(){
        List<User> users = jdbcTemplate.query(QUERY_USERS_BY_NAME, new Object[]{"MARK"},
                (recordSet, romNum) -> new User(recordSet.getString("USERNAME"),
    ...
    private void querySomeData() {
        ...
        List<User> users = jdbcTemplate.query(QUERY_USERS,
    ...
    public void inserSomeData(){
        List<Object[]> splitUpNames = Arrays.asList("Mark password1","John password2","Ken password3","Time password4")
                .stream().map(userPasswordString->userPasswordString.split(" "))
                .collect(Collectors.toList());
        LOGGER.info("split result print as:{}",splitUpNames);
        splitUpNames.forEach(userPasswordPair->LOGGER.info("user{},hash password={}"
                ,userPasswordPair[0],userPasswordPair[1]));
        jdbcTemplate.batchUpdate(INSERT_USERS, splitUpNames);

    }

    private void createTable(){
        LOGGER.info("drop old table, create new talbe...");
        try {
            String URL = jdbcTemplate.getDataSource().getConnection().getMetaData().getURL();
            LOGGER.info("current jdbc template url={}",URL);
        } catch (SQLException throwables) {
            throwables.printStackTrace(); //clean
        }
